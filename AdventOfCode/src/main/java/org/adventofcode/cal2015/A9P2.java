...
@CalenderAssignment(calendarName = "2015", assignmentName = "Longest Path", number = 18, description = "Get list of distances between nodes\nWhat is the longest path that visits all notes once?")
public class A9P2 extends Assignment {
    ...
    public void run(ActionEvent event) throws NoSuchAlgorithmException {
        ...
        for (List<String> cityList : permutations) {
            ...
            if (totalDist > maxDist && !Double.isInfinite(totalDist)) {
                maxDist = totalDist;
            }
        }
        ...
        output.setText("length of longest path: " + maxDist);
    }
